/**
 * Cloudflare Pages Function — GET/POST /voice-servers
 *
 * Reads and writes the appConfig/voiceServers document in Firestore via the
 * REST API using a Google service account — same pattern as feature-flags.js.
 * Stores the list of Dr. Tewari voice servers (name + URL) that admins have
 * published; the ePSA app itself reads the same document directly via the
 * public "get" rule in firestore.rules, not through here.
 *
...
export async function onRequestGet({ env, request }) {
  ...
  if (fsRes.status === 404) {
    return new Response(JSON.stringify(DEFAULT_SERVERS), { status: 200, headers: JSON_CT });
  }
  ...
  const parsed = parseFields(fsData.fields || {});
  return new Response(JSON.stringify({ servers: parsed.servers ?? DEFAULT_SERVERS.servers }), {
    status: 200,
    headers: JSON_CT,
  });
}
...
export async function onRequestPost({ env, request }) {
  ...
  if (!Array.isArray(body.servers)) {
    return errorResponse('Body must include a "servers" array', 400);
  }
  for (const s of body.servers) {
    if (typeof s?.name !== 'string' || typeof s?.url !== 'string' || !s.name.trim() || !s.url.trim()) {
      return errorResponse('Each server must have a non-empty "name" and "url"', 400);
    }
  }
  ...
  const merged = { servers: body.servers };

  const writeUrl = `${docUrl(sa.project_id)}?updateMask.fieldPaths=servers`;
...
function docUrl(projectId) {
  return `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/${VOICE_SERVERS_DOC}`;
}
